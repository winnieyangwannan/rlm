...
def parse_agent_output(response: str) -> dict[str, str]:
    ...
    Expected tags: <error_summary>, <root_cause>, <important_lesson>
    
    Returns:
        Dict mapping tag names to their content (stripped of whitespace)
    """
    sections = {}
    for tag in ["error_summary", "root_cause", "important_lesson"]:
        pattern = rf"<{tag}>(.*?)</{tag}>"
...
PROMPT_PATH = "/home/winnieyangwn/amaia-collab/apps/sea/envs/envs/mle_bench/prompts/gpt5-517-rlm.py"  # Path to the prompt file used in the evaluation
model_name = "gpt-5"  # Example model name
job_name = "common_invalid_errors_codebase_prompt"
...
2. **Diagnose Root Cause**: Focus on the most frequent error. Analyze the relevant rollout transcripts (the `rollout` column) to understand what the agent was doing when this error occurred. What is the likely cause of this error?

3. **Codebase Investigation**: Search the `codebase` to identify which code components are responsible for this error. Is it a bug in the evaluation code, or is the agent producing invalid outputs?

4. **Prompt Improvement**: This is the most important task.
   a) First, run `print(prompt)` to view the current prompt content
   b) Note that the prompt has an `{important_lesson}` placeholder section
   c) Write guidance that should go in `<important_lesson>` to prevent the top error

OUTPUT FORMAT:
Your final answer MUST be structured with these XML tags:

<error_summary>
Brief description of the top error (1-2 sentences)
</error_summary>

<root_cause>
Explanation of why this error occurs (2-3 sentences)
</root_cause>

<important_lesson>
The EXACT text to inject into the {important_lesson} placeholder in the prompt.
This should be actionable guidance for the ML agent to prevent the error.
Write it as if speaking directly to the ML agent.
...
"""
    ...
    if parsed.get("error_summary"):
        print("\n=== ERROR SUMMARY ===")
        print(parsed["error_summary"])
    
    if parsed.get("root_cause"):
        print("\n=== ROOT CAUSE ===")
        print(parsed["root_cause"])
    
    if parsed.get("important_lesson"):
        print("\n=== IMPORTANT LESSON (ready to inject into prompt) ===")
        print(parsed["important_lesson"])
        
        # Optionally save to file for easy copy-paste
        output_path = os.path.join(log_dir, f"{model_name}_{job_name}_{run_id}_important_lesson.txt")
        with open(output_path, "w") as f:
            f.write(parsed["important_lesson"])
        print(f"\n[Saved to: {output_path}]")
    else:
        print("\n[WARNING] No <important_lesson> section found in response.")
