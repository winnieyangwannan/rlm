...
# =============================================================================
run_id = 503
DATA_PATH = f"/checkpoint/agentic-models/winnieyangwn/amaia_dumps/{run_id}/trajectories/{run_id}_metadata.jsonl"
CODEBASE_PATH = "/checkpoint/agentic-models/winnieyangwn/amaia_dumps/503/code/2026_02_06_09_33_54"  # Path to codebase directory
CODEBASE_EXTENSIONS = [".py", ".md", ".yaml"]  # File extensions to include (e.g., [".py", ".ts", ".js"])
CONFIG_YAML_PATH = "/checkpoint/agentic-models/winnieyangwn/config/503.yaml"  # Path to the YAML config file used to run the evaluation
model_name = "gpt-5"  # Example model name
job_name = "common_invalid_errors_codebase"
log_dir = "/checkpoint/maui_sft/winnieyangwn/rlm_dumps/errors_codebase"
...
def get_row_count(path: str) -> int:
    ...
    if result.returncode != 0 or not result.stdout.strip():
        raise FileNotFoundError(f"Failed to count rows in {path}: {result.stderr.strip()}")
...
"""
    ...
    question = "Examine the ROLLOUT DATA, Among all invalid submissions, what are the top 5 most common evaluation error messages?  What are the likely causes to the top 5 error types respectively? Try to identify the root cause of the error by analyzing the code soluton in the ROLLOUT DATA. Suggest specific improvements to fix this error in future rollouts."
