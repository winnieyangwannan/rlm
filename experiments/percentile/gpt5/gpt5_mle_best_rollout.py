"""
Script to extract rollout_str from rollout with highest score.

This script:
1. Loads data from a JSONL file as pandas DataFrame
2. Finds the rollout with the highest score
3. Extracts the rollout_str and saves as .md file
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Extract rollout_str from medal rollouts")
    parser.add_argument(
        "--run_id",
        type=str,
        default="514",
...
def extract_and_save_rollout(
    ...
    """Extract rollout_str from medal rollouts and save as .md files.
...
def main():
    ...
    # 3. Extract rollout_str and save as .md file
    saved_files = extract_and_save_rollout(
        df=best_df,
    ...
    print(f"\nSaved {len(saved_files)} rollout files to {args.output_dir}")
