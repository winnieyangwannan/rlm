"""
Wrapper script to run MLE Bench comparison analysis for multiple tasks (v4 format).

v4 format: summarization files are in a flat directory:
  {summarization_dir}/*_{task_name}_*.md

This wrapper runs gpt5_mle_comparison_v4.py for each task in parallel.
"""
...
def has_regeneration_error(md_content: str) -> bool:
    """Check if the md content contains error messages or is invalid and requires regeneration."""
    ...
    if re.search(r"Error: Variable '[^']+' not found", md_content):
        return True
    # Check if report is too short (likely incomplete/invalid)
    if len(md_content.strip()) < 500:
        return True
    # Check for explicit invalid report markers
    if "This report is not valid" in md_content:
        return True
    return False
...
def run_single_task(
    ...
    # v4 format: summarization files are in flat directory with pattern *_{task_name}_*.md
    pattern = f"*_{task_name}_*.md"
    summarization_files = list(summarization_dir.glob(pattern))
    ...
    try:
        ...
        with FileLock(lock_file, timeout=0):
            ...
            # v4: output has "_comparison_report.md" suffix
...
def extract_task_names_from_summarization_dir(summarization_dir: Path) -> list[str]:
    """Extract unique task names from summarization .md files in a flat directory.
    
    Expected filename pattern: *_{task_name}_*.md
    """
    task_names = set()
    for md_file in summarization_dir.glob("*.md"):
        # Extract task name from filename pattern: prefix_taskname_suffix.md
        # Try to extract the task name between first underscore and last underscore before .md
        name = md_file.stem  # Remove .md
        parts = name.split("_")
        if len(parts) >= 3:
            # Assume format: model_jobname_runid_taskname.md or similar
            # Try to find task name by looking for known patterns
            # For robustness, we extract everything after the first few known prefixes
            # This is a heuristic - adjust based on actual naming convention
            task_name = "_".join(parts[3:]) if len(parts) > 3 else parts[-1]
...
def run_for_tasks(
    ...
    Args:
        task_names: List of task names to analyze. If None or empty, 
                    attempts to extract task names from summarization_dir.
        summarization_dir: Directory containing summarization .md files (flat structure).
    ...
    if not task_names:
        # Try to extract task names from existing summarization files
        task_names = extract_task_names_from_summarization_dir(summarization_path)
        print(f"Found {len(task_names)} tasks from summarization files in {summarization_dir}")
    
    if not task_names:
        print("No tasks to process. Please provide task_names or ensure summarization_dir has .md files.")
        return
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                run_single_task, task, script_path, summarization_path, log_dir_path, run_id,
...
if __name__ == "__main__":
    ...
    summarization_dir = "/checkpoint/maui_sft/winnieyangwn/rlm_dumps/summarization/mle-30"
