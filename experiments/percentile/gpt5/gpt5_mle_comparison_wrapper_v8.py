"""
Wrapper script to run MLE Bench comparison analysis for multiple tasks (v8 format).

v8 format: summarization files are organized in task subfolders:
  {summarization_dir}/{task_name}/*.md

v8 changes:
- Compares only highest tier vs lowest tier (instead of adjacent tier pairs)
- Saves single comparison report per task (no need to assemble multiple pairs)
"""
...
def has_regeneration_error(md_content: str) -> bool:
    ...
    # A valid comparison report should have substantial content
...
def run_single_task(
    ...
    # v8 format: summarization files are in task subfolders
    ...
    try:
        ...
        with FileLock(lock_file, timeout=0):
            ...
            # v8: combined report has "_comparison_report.md" suffix
...
def run_for_tasks(
    ...
    if not task_names:
        # v8 format: task names are subfolder names in summarization_dir
...
if __name__ == "__main__":
    ...
    mle_30_7 = ["kuzushiji-recognition",
                ...
                "mlsp-2013-birds",]  
    # v8 script path
    script_path = "/home/winnieyangwn/rlm/experiments/percentile/gpt5/gpt5_mle_comparison_v8.py"
    ...
    mle_improve = ["hotel-id-2021-fgvc8",
                   "h-and-m-personalized-fashion-recommendations",
                   "cassava-leaf-disease-classification",
                   "kuzushiji-recognition",
                   "whale-categorization-playground",
