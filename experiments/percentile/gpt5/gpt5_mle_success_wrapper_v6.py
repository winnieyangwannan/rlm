"""
Wrapper script to run MLE Bench comparison analysis for multiple tasks (v6 format).

v6 format: summarization files are organized in task subfolders:
...
# Get rlm project root for PYTHONPATH in subprocesses
RLM_ROOT = str(Path(__file__).resolve().parents[3])
...
def run_single_task(
    ...
    # v6 format: summarization files are in task subfolders
    ...
    try:
        ...
        with FileLock(lock_file, timeout=0):
            ...
            env = os.environ.copy()
            env["PYTHONPATH"] = RLM_ROOT + (":" + env.get("PYTHONPATH", "") if env.get("PYTHONPATH") else "")
            subprocess.run(
                [
                    sys.executable,
                    script_path,
...
def run_for_tasks(
    ...
    if not task_names:
        # v6 format: task names are subfolder names in summarization_dir
...
if __name__ == "__main__":
    ...
    # mle_tsest = ["tweet-sentiment-extraction",]
    ...
    mle_30_7 = ["kuzushiji-recognition",
                ...
                "multi-modal-gesture-recognition",
                "mlsp-2013-birds",]  
    script_path = "/home/winnieyangwn/rlm/experiments/percentile/gpt5/gpt5_mle_success_v6.py"
    summarization_dir = "/checkpoint/maui_sft/winnieyangwn/rlm_dumps/summarization/mle-30/v6"
    log_dir = "/checkpoint/maui_sft/winnieyangwn/rlm_dumps/success/mle-30/v6"
    run_id = 514
