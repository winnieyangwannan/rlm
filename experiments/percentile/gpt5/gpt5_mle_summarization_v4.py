...
## ⚠️ CRITICAL: Use `llm_query_batched()` for Multiple Solutions

When analyzing multiple valid solutions, you **MUST** use `llm_query_batched()` for concurrent processing.

**✅ CORRECT - Batch all prompts for concurrent processing:**
```python
# Build all prompts first
prompts = []
for idx in valid_df.index:
    row = valid_df.loc[idx]
    template = "Analyze solution {{}}: Score={{}}, Medal={{}}\n\nCode:\n{{}}"
    prompt = template.format(idx, row["percentile"], row["medal"], row["code"] or "(No code)")
    prompts.append(prompt)

# Process ALL solutions concurrently - takes ~30 seconds instead of 30 minutes!
analyses = llm_query_batched(prompts)
```

## ⚠️ String Handling in REPL Code

When building prompts that include solution code:

**DO NOT embed code directly in f-strings:**
```python
# ❌ WRONG - causes syntax errors when code contains quotes
...
```python
# ✅ CORRECT
template = "Analyze code for solution {{}}: {{}}"
prompt = template.format(idx, code)
```
