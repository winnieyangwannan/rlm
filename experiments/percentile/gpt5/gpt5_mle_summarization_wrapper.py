...
def run_single_task(task_name: str, script_path: str, log_dir: Path) -> tuple[str, bool, str]:
    ...
    try:
        ...
        with FileLock(lock_file, timeout=0):
            ...
            if existing_md:
                # Check if the md file contains errors requiring regeneration
                if has_regeneration_error(existing_md[0]):
                    # Delete both md and jsonl files, then regenerate
...
if __name__ == "__main__":
    ...
    script_path = "/home/winnieyangwn/rlm/experiments/percentile/gpt5/gpt5_mle_summarization_v4.py"
    log_dir = Path("/checkpoint/maui_sft/winnieyangwn/rlm_dumps/summarization/mle-45/v4-2")
     # 9 + 11 + 12 + 13
    run_for_tasks(mle_45, max_workers=10)  # Adjust max_workers based on API rate limits
