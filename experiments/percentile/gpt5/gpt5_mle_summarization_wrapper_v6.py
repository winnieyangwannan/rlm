...
def has_regeneration_error(md_file: Path) -> bool:
    ...
    Synced with has_error_in_md() in gpt5_mle_summarization_v6.py
    """
    try:
...
def run_single_task(task_name: str, script_path: str, log_dir: Path) -> tuple[str, bool, str]:
    ...
    # v6.py outputs to log_dir/task_name
    task_output_dir = log_dir / task_name
    task_output_dir.mkdir(parents=True, exist_ok=True)
    lock_file = task_output_dir / f".{task_name}.lock"
    ...
    try:
        ...
        with FileLock(lock_file, timeout=0):
            ...
            # v6.py filename format: {model}_{job_name}_{run_id}_{task_name}_rollout{idx}_{timestamp}_{uuid}.md
            existing_md = list(task_output_dir.glob(f"*_{task_name}_rollout*_*.md"))
            if existing_md:
                ...
                if has_regeneration_error(most_recent):
                    # Delete all md files, then regenerate
...
if __name__ == "__main__":
    ...
    mle_test = ["cassava-leaf-disease-classification"]

    script_path = "/home/winnieyangwn/rlm/experiments/percentile/gpt5/gpt5_mle_summarization_v6.py"
    log_dir = Path("/checkpoint/maui_sft/winnieyangwn/rlm_dumps/summarization/mle-30/v6")
     # 9 + 11 + 12 + 13
    run_for_tasks(mle_test, max_workers=5)  # Adjust max_workers based on API rate limits
